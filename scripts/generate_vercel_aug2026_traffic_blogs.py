...
"""Generate Aug 2026 Vercel traffic-gap blog posts (≥3 min each)."""
...
BLOGS = ROOT / "blogs"
DATA = ROOT / "data" / "vercel-aug2026-traffic-posts.json"
...
def main() -> None:
    ...
    for post in posts:
        ...
        if words < 600:
            raise SystemExit(f"Post too short for 3+ min read: {post['slug']} ({words} words)")
        ensure_image(post)
