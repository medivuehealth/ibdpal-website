...
"""Generate short blog posts from Vercel top-pages gap analysis (Jul 2026)."""
...
BLOGS = ROOT / "blogs"
DATA = ROOT / "data" / "vercel-page-gap-posts.json"
...
def ensure_image(post: dict) -> None:
    ...
    if existing:
        ...
        return
    # Try common alternate names in asset dir parents
    for folder in sorted((BLOGS / "assets").glob("*")):
        if not folder.is_dir():
            continue
        jpgs = sorted(folder.glob("*.jpg"))
        if jpgs:
            shutil.copy(jpgs[0], dest)
            return
    if FALLBACK.exists():
...
def main() -> None:
    ...
    for post in posts:
        ...
        if not (asset / wanted).exists():
            jpgs = sorted(asset.glob("*.jpg")) if asset.exists() else []
            if jpgs:
