# Blog image credits

Images in the June 2026 lifestyle posts are from [Unsplash](https://unsplash.com/license) and [Pexels](https://www.pexels.com/license/) (free use). Download via `images.unsplash.com` or Pexels CDN — not the `/photos/{id}/download` HTML redirect.

| Folder | Files | Typical subjects |
|--------|-------|------------------|
| `low-residue/` | `low-residue_*.jpg` | Gentle meals, soup, home cooking |
| `travel-ibd/` | `travel-ibd_*.jpg` | Packing, travel meals, planning |
| `biologics-basics/` | `biologics_*.jpg` | Wellness, care conversations |
| `ibd-kids/` | `ibd-kids_*.jpg` | Family time, parents and children |
| `stress-wellness/` | `stress_*.jpg` | Calm, rest, nature |
| `sleep-rest/` | `sleep_*.jpg` | Bedroom rest, relaxation |
| `workplace-school/` | `workplace_1.jpg` | Office / remote work (Pexels) |
| `insurance-basics/` | `insurance_1.jpg` | Planning, paperwork (Pexels) |
| `ostomy-basics/` | `ostomy_1.jpg` | Supportive conversation (Pexels) |
| `flare-48h/` | `flare_1.jpg` | Restful bedroom (Unsplash) |
| `caregiver-partner/` | `caregiver_1.jpg` | Family / partner support (Pexels) |
| `fodmap-ibd/` | `fodmap_1.jpg` | Vegetables and healthy meals (Unsplash) |
| `high-school-ibd/` | `highschool_1.jpg` | Students / campus (Unsplash) |
| `teen-nutrition/` | `teen_nutrition_1.jpg` | School lunch / balanced meal (Unsplash) |
| `anti-inflammatory-ibd/` | `anti_inflammatory_1.jpg` | Mediterranean-style salad (Unsplash) |
| `exercise-ibd/` | `exercise_1.jpg` | Outdoor running / activity (Unsplash) |
| `vitamins-ibd/` | `vitamins_1.jpg` | Sunlight / wellness (Unsplash) |
| `teen-social-ibd/` | `teen_social_1.jpg` | Friends outdoors (Unsplash) |
| `protein-meals-ibd/` | `protein_meals_1.jpg` | Fish dinner plate (Unsplash) |

Photographers are credited on each photo’s Unsplash or Pexels page. To replace an image, use another asset marked with the standard Unsplash or Pexels license (not Unsplash+).
